...
function NovaAnaliseContent({ requestedPatientId }: { requestedPatientId: string | null }) {
  ...
  const resetChat = () => {
    setChatMessages([]);
    setChatInput('');
    setChatImportedCase(null);
    setChatShowPicker(false);
    setChatCaseSearch('');
    setChatCaseId(null);
    setChatError('');
  ...
  const chatFirstName = user?.name?.split(' ')[0] ?? '';
  const chatSuggestions = ['Hipótese diagnóstica', 'Plano de intervenção', 'Risco no caso', 'Dúvida técnica'];
  const chatPatient = patients.find(p => p.id === chatSelectedPatientId) ?? null;
            ...
            <div className="flex flex-col items-center justify-center flex-1 gap-6 px-4">
              ...
              <div className="w-full max-w-xl space-y-2">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white shadow-sm px-4 py-3">
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
                    <Users className="w-3 h-3" /> Paciente <span className="text-slate-300 font-normal normal-case tracking-normal">(opcional)</span>
                  </p>
                  <PatientSelector
                    patients={patients}
                    selectedId={chatSelectedPatientId}
                    onSelect={id => setChatSelectedPatientId(id)}
                    onNewPatient={() => setShowPatientModal(true)}
                  />
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white shadow-sm px-4 py-3">
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1.5">
                    <FolderHeart className="w-3 h-3" /> Importar caso <span className="text-slate-300 font-normal normal-case tracking-normal">(opcional)</span>
                  </p>
                  <div className="relative">
                    {chatImportedCase ? (
                      <div className="flex items-center gap-2">
                        <div className="flex-1 flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-xl text-xs font-semibold text-blue-700">
                          <FolderHeart className="w-3.5 h-3.5 shrink-0" />
                          <span className="truncate">{chatImportedCase.title}</span>
                        </div>
                        <button onClick={() => setChatImportedCase(null)} className="p-1.5 text-slate-400 hover:text-slate-600 transition-colors shrink-0"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                      <button
                        onClick={() => setChatShowPicker(v => !v)}
                        className="w-full flex items-center justify-between px-3 py-2 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/40 text-slate-500 hover:text-blue-700 text-xs font-medium transition-all"
                      >
                        <span>Selecionar caso do histórico…</span>
                        <ChevronDown className={`w-3.5 h-3.5 transition-transform ${chatShowPicker ? 'rotate-180' : ''}`} />
                      </button>
                    )}
                    {chatShowPicker && !chatImportedCase && (
                      <div className="absolute left-0 top-full mt-1 w-full bg-white border border-slate-200 rounded-2xl shadow-lg z-50 overflow-hidden">
                        <div className="p-3 border-b border-slate-100">
                          <div className="relative">
                            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                            <input type="text" value={chatCaseSearch} onChange={e => setChatCaseSearch(e.target.value)} placeholder="Buscar caso..." className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-3 py-1.5 text-xs text-slate-700 placeholder-slate-400 outline-none" autoFocus />
                          </div>
                        </div>
                        <div className="overflow-y-auto max-h-48">
                          {cases.filter(c => c.approach_used !== 'Chat' && c.title.toLowerCase().includes(chatCaseSearch.toLowerCase())).length === 0 && <p className="text-xs text-slate-400 text-center py-5">Nenhum caso encontrado.</p>}
                          {cases.filter(c => c.approach_used !== 'Chat' && c.title.toLowerCase().includes(chatCaseSearch.toLowerCase())).map(c => (
                            <button key={c.id} onClick={() => { setChatImportedCase(c as unknown as ChatCase); setChatShowPicker(false); setChatCaseSearch(''); }} className="w-full flex items-center gap-2.5 px-4 py-3 hover:bg-blue-50 text-left transition-colors border-b border-slate-50 last:border-0">
                              <div className="w-7 h-7 rounded-lg bg-blue-600 text-white flex items-center justify-center text-[9px] font-bold shrink-0">{c.approach_used?.slice(0, 3).toUpperCase() ?? '?'}</div>
                              <div className="min-w-0"><p className="text-xs font-semibold text-slate-800 truncate">{c.title}</p><p className="text-[10px] text-slate-400 truncate">{c.approach_used}</p></div>
                            </button>
                          ))}
                        </div>
                      </div>
                ...
                <div className="chat-input-box rounded-2xl border border-slate-200 bg-white shadow-sm px-4 pt-3.5 pb-3">
                  ...
                  <div className="flex justify-end mt-2">
                    <button
                      ...
                      className="w-8 h-8 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-30 disabled:cursor-not-allowed text-white flex items-center justify-center transition-all shadow-[0_4px_12px_rgba(37,99,235,0.25)]"
                    >
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {chatSuggestions.map(s => (
                  <button
                    key={s}
                    onClick={() => { setChatInput(s); setTimeout(() => chatInputRef.current?.focus(), 0); }}
                    className="px-3.5 py-1.5 rounded-full border border-slate-200 text-[12px] text-slate-500 hover:text-slate-700 hover:border-slate-300 bg-white transition-all"
                  >
                    {s}
                  </button>
                ))}
            ...
            <>
              ...
              <div className="flex items-center gap-2 px-5 py-2.5 border-b border-slate-100 shrink-0">
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  {chatPatient && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 border border-blue-200 rounded-full text-[11px] font-semibold text-blue-700">
                      <Users className="w-3 h-3" />{chatPatient.pseudonym}
                    </div>
                  )}
                  {chatImportedCase && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 border border-slate-200 rounded-full text-[11px] font-medium text-slate-600 max-w-[180px]">
                      <FolderHeart className="w-3 h-3 shrink-0" /><span className="truncate">{chatImportedCase.title}</span>
                    </div>
                  )}
                  {!chatPatient && !chatImportedCase && (
                    <span className="text-[11px] text-slate-400">Chat livre</span>
                  )}
                </div>
                <button
                  onClick={resetChat}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50 text-[11px] font-semibold text-slate-500 hover:text-blue-700 transition-all shrink-0"
                >
              ...
              <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start items-start gap-3'}`}>
                    ...
                    <div className={
                      msg.role === 'user'
                        ? 'max-w-[78%] px-4 py-3 bg-slate-100 rounded-2xl text-sm text-slate-800 leading-relaxed'
                        : 'max-w-[78%] text-sm text-slate-700 leading-relaxed'
                    }>
                      {msg.content}
                    </div>
                  </div>
                    ...
                    <div className="relative">
                      {chatImportedCase ? (
                        <div className="flex items-center gap-1.5">
                          <div className="flex items-center gap-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded-lg text-[10px] font-semibold text-blue-700 max-w-[130px]">
                            <FolderHeart className="w-3 h-3 shrink-0" />
                            <span className="truncate">{chatImportedCase.title}</span>
                          </div>
                          <button onClick={() => setChatImportedCase(null)} className="text-slate-400 hover:text-slate-600 transition-colors"><X className="w-3.5 h-3.5" /></button>
                        </div>
                      ) : (
                        <button onClick={() => setChatShowPicker(v => !v)} className="w-8 h-8 rounded-xl border border-slate-200 hover:border-slate-300 flex items-center justify-center text-slate-500 hover:text-slate-700 transition-all text-lg leading-none">+</button>
                      )}
                      {chatShowPicker && !chatImportedCase && (
                        <div className="absolute left-0 bottom-full mb-2 w-72 bg-white border border-slate-200 rounded-2xl shadow-lg z-50 overflow-hidden">
                          <div className="p-3 border-b border-slate-100">
                            <div className="relative">
                              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
                              <input type="text" value={chatCaseSearch} onChange={e => setChatCaseSearch(e.target.value)} placeholder="Buscar caso..." className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-3 py-1.5 text-xs text-slate-700 placeholder-slate-400 outline-none" autoFocus />
                          ...
                          <div className="overflow-y-auto max-h-56">
                            {cases.filter(c => c.approach_used !== 'Chat' && c.title.toLowerCase().includes(chatCaseSearch.toLowerCase())).length === 0 && <p className="text-xs text-slate-400 text-center py-6">Nenhum caso encontrado.</p>}
                            {cases.filter(c => c.approach_used !== 'Chat' && c.title.toLowerCase().includes(chatCaseSearch.toLowerCase())).map(c => (
                              <button key={c.id} onClick={() => { setChatImportedCase(c as unknown as ChatCase); setChatShowPicker(false); setChatCaseSearch(''); }} className="w-full flex items-center gap-2.5 px-4 py-3 hover:bg-blue-50 text-left transition-colors border-b border-slate-50 last:border-0">
                                <div className="w-7 h-7 rounded-lg bg-blue-600 text-white flex items-center justify-center text-[9px] font-bold shrink-0">{c.approach_used?.slice(0, 3).toUpperCase() ?? '?'}</div>
                                <div className="min-w-0"><p className="text-xs font-semibold text-slate-800 truncate">{c.title}</p><p className="text-[10px] text-slate-400 truncate">{c.approach_used}</p></div>
